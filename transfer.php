	...
	<tr class="tableheader">
		...
		<tr>
			<td colspan="4">
				...
				<?php  
					if($action=='Transfer' &&  !isset($_GET['to']))
						$filter="";
					else
						$filter="and for_customer in(". $_SESSION['item_group'] .")";
					$sql="select * from items where is_active=1 $filter order by 1 ";
					$rows=getRows($sql);
					$cnt=0;
					$grp="";
					foreach($rows as $row)
					{
						$cnt++;
						if($grp != $row['item_group'])
						{
							$grp = $row['item_group'];
							echo "<tr><td colspan=2><strong>$grp</strong></td></tr>";
						}
				?>	
					<tr>
